...
describe 'ZsyFuzzyFinder', ->
  ...
  describe "file-finder behavior", ->
    ...
    describe "toggling", ->
      describe "when the project has multiple paths", ->
        it "shows or hides the zsy-fuzzy-finder and returns focus to the active editor if it is already showing", ->
        ...
        it "ignores paths that match entries in config.zsy-fuzzy-finder.ignoredNames", ->
          atom.config.set("zsy-fuzzy-finder.ignoredNames", ["sample.js", "*.txt"])
  ...
  describe "buffer-finder behavior", ->
    describe "toggling", ->
      describe "when there are pane items with paths", ->
        ...
        it "shows the ZsyFuzzyFinder if it isn't showing, or hides it and returns focus to the active editor", ->
          ...
          expect(workspaceElement.querySelector('.zsy-fuzzy-finder')).toHaveFocus()
        ...
        it "serializes the list of paths and their last opened time", ->
          ...
          runs ->
            atom.packages.deactivatePackage('zsy-fuzzy-finder')
            states = _.map atom.packages.getPackageState('zsy-fuzzy-finder'), (path, time) -> [ path, time ]
    ...
    describe "when a path selection is confirmed", ->
      ...
      describe "when the active pane does not have an item for the selected path and zsy-fuzzy-finder.searchAllPanes is false", ->
        it "adds a new item to the active pane for the selected path", ->
          ...
          expectedPath = atom.project.getDirectories()[0].resolve('sample.txt')
          bufferView.confirmed({filePath: expectedPath}, atom.config.get 'zsy-fuzzy-finder.searchAllPanes')
      ...
      describe "when the active pane does not have an item for the selected path and zsy-fuzzy-finder.searchAllPanes is true", ->
        beforeEach ->
          atom.config.set("zsy-fuzzy-finder.searchAllPanes", true)
        ...
        it "switches to the pane with the item for the selected path", ->
          ...
          expectedPath = atom.project.getDirectories()[0].resolve('sample.txt')
          bufferView.confirmed({filePath: expectedPath}, searchAllPanes: atom.config.get('zsy-fuzzy-finder.searchAllPanes'))
  ...
  describe "preserve last search", ->
    ...
    it "preserves last search when the config is set", ->
      atom.config.set("zsy-fuzzy-finder.preserveLastSearch", true)
